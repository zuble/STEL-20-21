...
int main(int argc, char* argv[]){
  ...
  if(argc < 2)  {
    perror("need another file name to save data");
    return -1;
  }
  ...
  int n_events;
  float c = 0 , summation = 0 , curr_time = 0;
  int *histogram = (int*)malloc(HIST_SIZE*sizeof(int));
  lista *event = NULL;

  srand(time(NULL));
  event = adicionar(event , ARRIVAL , 0);

  for (n_events = 0; n_events < N_EVENTS; n_events++) {

    c = - (1 / ((float)LAMBDA)) * log( rand01() );
    curr_time = event->tempo + c;
    event = rem(event);
    event = adicionar(event, ARRIVAL, curr_time);
